...
echo -e "configure_finalverse.sh" > $LOG
...
if [ $WANTS_CONFIG -eq $TRUE ] ; then
    ...
    if [ $WANTS_CRASHREPORTING -eq $TRUE ] ; then
        ...
        if [ $TARGET_PLATFORM == "linux" ] ; then
            VIEWER_SYMBOL_FILE="${BUILD_DIR}/newview/finalverse-symbols-${TARGET_PLATFORM}-${AUTOBUILD_ADDRSIZE}.tar.bz2"
        else
            VIEWER_SYMBOL_FILE="${BUILD_DIR}/newview/$BTYPE/finalverse-symbols-${TARGET_PLATFORM}-${AUTOBUILD_ADDRSIZE}.tar.bz2"
        fi
        CRASH_REPORTING="-DRELEASE_CRASH_REPORTING=ON"
        if [ ! -z $CHANNEL_SIMPLE ]
        then
            CRASH_REPORTING="$CRASH_REPORTING -DUSE_BUGSPLAT=On -DBUGSPLAT_DB=finalverse_"`echo $CHANNEL_SIMPLE | tr [:upper:] [:lower:] | sed -e 's/x64//' | sed -e 's/_//g'`
        fi
    else
    ...
    if [ $TARGET_PLATFORM == "windows" -a $USE_VSTOOL -eq $TRUE ] ; then
        ...
        ../indra/tools/vstool/VSTool.exe --solution finalverse.sln --startup finalverse-bin --workingdir finalverse-bin "..\\..\\indra\\newview" --config $BTYPE
    fi
fi
...
if [ $WANTS_BUILD -eq $TRUE ] ; then
    ...
    if [ $TARGET_PLATFORM == "darwin" ] ; then
        ...
        xcodebuild -configuration $BTYPE -project finalverse.xcodeproj $JOBS 2>&1 | tee -a $LOG
    elif [ $TARGET_PLATFORM == "linux" ] ; then
        if [ $JOBS == "0" ] ; then
        ...
        msbuild.exe finalverse.sln /p:Configuration=${BTYPE} /flp:LogFile="logs\\finalverseBuild_win-${AUTOBUILD_ADDRSIZE}.log" \
                    /flp1:"errorsonly;LogFile=logs\\finalverseBuild_win-${AUTOBUILD_ADDRSIZE}.err" /p:Platform=${AUTOBUILD_WIN_VSPLATFORM} /t:Build /p:useenv=true \
